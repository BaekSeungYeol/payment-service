...
import me.whitewin.paymentservice.payment.domain.PaymentEventMessagePublisher
...
class R2DBCPaymentStatusUpdateRepository(
    ...
    private fun insertPaymentHistory(
        ...
        return databaseClient.sql(INSERT_PAYMENT_HISTORY_ORDER(valueClauses))
            .fetch()
            .rowsUpdated()
    }
            ...
            .handle { paymentOrder, sink ->
                when(paymentOrder.second) {
                    ...
                    PaymentStatus.SUCCESS.name -> {
                        sink.error(PaymentAlreadyProcessedException(status = PaymentStatus.SUCCESS, message = "이미 처리 성공한 결제입니다."))
                    }
                    PaymentStatus.FAILURE.name -> {
                        sink.error(PaymentAlreadyProcessedException(status = PaymentStatus.FAILURE, message = "이미 처리 실패된 결제입니다."))
                    }
                }
            }
            .collectList()
    ...
    companion object {
        ...
        val INSERT_PAYMENT_HISTORY_ORDER  = fun(valueClauses: String) = """
